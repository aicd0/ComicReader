...
internal class InternalServer : IServerContext
{
    ...
    private class ProcessCallback(InternalServer server, IPropertyContext context) : IProcessCallback
    {
        ...
        public void PostOnServerThread(bool completed, Action? action)
        {
            if (completed && Interlocked.CompareExchange(ref _completed, 1, 0) != 0)
            {
            ...
            {
                PerformAction(completed, action);
            }
            else
            {
        ...
        private void PerformAction(bool completed, Action? action)
        {
            if (action is not null)
            {
                context.HandleProcessCallback(action);
            }
