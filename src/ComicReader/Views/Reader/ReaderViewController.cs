...
internal class ReaderViewController
{
    ...
    public static readonly float MAX_ZOOM = 250F;
    public static readonly float MIN_ZOOM = 90F;
    public static readonly float FORCE_CONTINUOUS_ZOOM_THRESHOLD = 105F;
    private static readonly int MIN_PRELOAD_FRAMES_BEFORE = 5;
    private static readonly int MAX_PRELOAD_FRAMES_BEFORE = 10;
    private static readonly int MIN_PRELOAD_FRAMES_AFTER = 5;
    private static readonly int MAX_PRELOAD_FRAMES_AFTER = 10;
    ...
    public delegate void CommonListener();
    private event CommonListener OnLoaded;

    public void SetOnLoadedListener(CommonListener listener)
    {
        OnLoaded = null;
        OnLoaded += listener;
    }
    ...
    private void UpdateImagesInternal(bool remove_out_of_view = true)
    {
        ...
        int preload_window_end = Math.Min(frame + MAX_PRELOAD_FRAMES_AFTER, DataSource.Count - 1);
        ...
        int check_window_end = Math.Min(frame + MIN_PRELOAD_FRAMES_AFTER, DataSource.Count - 1);
    ...
    // Events - Common
    private readonly Utils.CancellationLock m_ContainerLoadedLock = new();
    private readonly Utils.CancellationLock m_PageRearrangeLock = new();
    ...
    public async Task OnContainerLoaded(ReaderFrameViewModel ctx)
    {
        ...
        await m_ContainerLoadedLock.LockAsync(async delegate (CancellationLock.Token token)
        {
            ...
            if (!LoadedInitialPage && LoadedFirstPage)
            {
                ...
                LoadedInitialPage = SetScrollViewer2(null, _initialPage, true, "JumpToInitialPage");
#if DEBUG_LOG_LOAD
